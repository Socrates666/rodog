...
*      Author: mokhwasomssi
       fixed by Socrates666
*/
#pragma once

#include <string.h>
#include "driver/i2c_master.h"			// header from stm32cubemx code generate
#include <stdbool.h>
#include "esp_log.h"
...
typedef struct{
    ...
    esp_err_t (*transmit_data_icm20948)(i2c_master_dev_handle_t i2c_dev, const uint8_t *write_buffer, size_t write_size, int xfer_timeout_ms);
    esp_err_t (*receive_data_icm20948)(i2c_master_dev_handle_t i2c_dev, uint8_t *read_buffer, size_t read_size, int xfer_timeout_ms);
}icm20948_cfg_t;
