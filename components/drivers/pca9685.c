...
static pca9685_config_t pca9685_dev;      // use config only
static uint16_t pca9685_frequency = 0;    // store frequency separately

// Internal helper functions
static esp_err_t pca9685_write_register(uint8_t reg, uint8_t value) {
    uint8_t data[2] = {reg, value};
    return pca9685_dev.transmit_data_pca9685(pca9685_dev.dev_handle, data, 2, 1000);
}
...
static esp_err_t pca9685_read_register(uint8_t reg, uint8_t *value) {
    ...
    esp_err_t ret = pca9685_dev.transmit_data_pca9685(pca9685_dev.dev_handle, &reg, 1, 1000);
    if (ret != ESP_OK) {
        return ret;
    }
    return pca9685_dev.receive_data_pca9685(pca9685_dev.dev_handle, value, 1, 1000);
}
...
static esp_err_t pca9685_write_registers(uint8_t reg, const uint8_t *data, size_t len) {
    ...
    esp_err_t ret = pca9685_dev.transmit_data_pca9685(pca9685_dev.dev_handle, data_with_reg, len + 1, 1000);
    free(data_with_reg);
    return ret;
}
...
static esp_err_t pca9685_read_registers(uint8_t reg, uint8_t *data, size_t len) {
    ...
    return pca9685_dev.receive_data_pca9685(pca9685_dev.dev_handle, data, len, 1000);
}
...
esp_err_t pca9685_init(const pca9685_config_t *config) {
    ...
    // Copy configuration
    memcpy(&pca9685_dev, config, sizeof(pca9685_config_t));
    pca9685_frequency = 0;  // Will be set after frequency configuration
